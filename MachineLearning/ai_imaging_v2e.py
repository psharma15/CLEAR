...
if __name__ == '__main__':
    ...
    nRecv = 4
    nFreq = 50
    nTagSkip = 4 # Skip 4 tags. Make corresponding model changes.
    ...
    pickLocTag = [11,17,13,18,19,15,9,8,7]
    ...
    ***************************************************************************
    ...
    for epoch in range(EPOCHS):
        ...
        if (epoch+1)%1 == 0:
            ...
            with torch.set_grad_enabled(False):    
                ...
                print('Epoch: %i' %(epoch+1),', Train loss: %0.3f' %(float(train_loss)),', Train Accuracy: %0.2f' %train_accuracy)
                print('Epoch: %i' %(epoch+1),', Test loss: %0.3f' %(float(test_loss)),', Test Accuracy: %0.2f' %test_accuracy)
                test_loss_n_epoch.append(float(test_loss))
                test_acc_n_epoch.append(test_accuracy)
               
    # ------------------------------------------------------------------------------------------------------
    # Some sanity checks by looking at the data
    y = Y_test.data.numpy()              # Test data: As taken from above, same shuffled as y_pred.
    y_pred = symbol_test.data.numpy()    # Predicted symbol
    ...
    y_totaldata = Y.data.numpy() # This is shuffled if totaldata_loader Dataset has shuffle, so correct implementation
    ...
    tempWrongDataName = [i.tolist() for i in (data['dataNames'][wrongDataIdxTotal])]
    wrongDataName = []
    for i in tempWrongDataName:
        #print(i)
        for j in i:
            wrongDataName.append(j.tolist())