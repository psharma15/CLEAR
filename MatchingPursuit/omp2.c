...
void main(void){
  ...
  int *ic, *id, num, iclust[100],*final;
  double *dimage,*centroid;
  unsigned int iseed,npoints=0;
  float wn, pi, c, pse, dist, amp, thresh=2.0e-14;
  float *freqs,*rxx,*rxy,*rxz,*txx,*txy,*txz;
  float *image, x, y, z, *d1, *d2, gamma;
  float *stdev;
  fcomplex *sparams, *sparams2, poff, sum, fac;
  fcomplex *rcal, *tcal, *ccal;
  fcomplex *s0, *r0, *q0, *p0;
  float tk,tkold,fac1,fac2,anorm;
  fcomplex *g, *gg, *gp, *pe, *ata, *atai, zero, one, mone; 
  fcomplex *xk,*xp, calpha;
  float tnew,told,alpha,rad,rad2,arg,corr,cmax,ang,frac;
  int inc, imax, imaxp;
  int *lib, im;
  int *bad_data, *bad_cal;
  long pointer, pointer2, dptr, mptr;
  unsigned long nn[3]={NX,NY,NZ};
  float sinx, cosx;

  //float x0=-0.5, x1=0.5, y0=-0.5, y1=0.5, z0=-0.25, z1=0.25;
  //float x0=0.0, x1=0.9, y0=0.0, y1=0.9, z0=0.0, z1=0.3;
  //float x0=0.1, x1=0.9, y0=0.1, y1=0.9, z0=0.1, z1=0.7;
  //float x0=0.4, x1=3.6, y0=0.4, y1=3.6, z0=0.4, z1=2.1;
  //float x0=0.0, x1=3.5, y0=0.0, y1=3.5, z0=0.0, z1=3.0;
  float x0=-1.0, x1=4.5, y0=-1.0, y1=4.5, z0=-1.0, z1=4.0;

  pi = 4.0*atanf(1.0);
  c = 2.998e8;

  image=(float*) calloc(NX*NY*NZ,sizeof(float));  
  ic = (int*) calloc(NX*NY*NZ,sizeof(int)); 
  id = (int*) calloc(NX*NY*NZ,sizeof(int)); 
  dimage = (double*) malloc(NPMAX*3*sizeof(double));
  centroid = (double*) malloc(NCMAX*3*sizeof(double));
  final = (int*) malloc(NPMAX*sizeof(int));
  ...
  ndat = diagnose("phStdDevObj.mat");
  stdev = (float*)malloc(sizeof(float)*ndat);  // st. dev. meas.
  for(i=0;i<ndat;i++)
    stdev[i]=pr[i];
  ...
  bad_data=(int*)calloc(ndat,sizeof(int));
  bad_cal=(int*)calloc(ndat,sizeof(int)); 

  sparams = (fcomplex*)malloc(sizeof(fcomplex)*ndat);
  for(i=0;i<ndat;i++){
    //printf("%i %i %f %f \n",i,ndat,pr[i],pim[i]);
    sparams[i]=Complex(SCALE*pr[i],SCALE*pim[i]); 
    if(Cmod(sparams[i])==0.0)
      bad_cal[i]=1;
      //printf("Empty cal data in index %i \n",i);
    //printf("%i %i %f %f \n",i,ndat,pr[i],pim[i]);
  } 

  /*

  // comprehensive cal for each freq, tag, rx
  
  for(j=0;j<nfreq;j++){
    wn = 2.0*pi*freqs[j]/c;
    for(k=0;k<nrx;k++){  // loop over rx
      for(i=0;i<ntag;i++){ // loop over tags
	pse = -wn*dist(rxx[k],rxy[k],rxz[k],txx[i],txy[i],txz[i]);
	fac = Complex(cos(pse),sin(pse));
	dptr = (long)(i+ntag*(k+nrx*j));	
	ccal[dptr]=Cmul(fac,Conjg(sparams[dptr]));
	if(Cabs(ccal[dptr])>0.0)
	  ccal[dptr]=RCdiv(ccal[dptr],Cabs(ccal[dptr]));
      }
    }
  }
  ...
  */
  ...
  for(j=0;j<nfreq;j++){
	...
	if(Cmod(sum)>0.0) // in case of doctored data
	  tcal[i+ntag*j]=RCdiv(sum,sqrt(Cmod(sum)));
	else
	  tcal[i+ntag*j]=Complex(1.0,0.0);
  ...
  for(i=0;i<ndat;i++){
    sparams2[i]=Complex(SCALE*pr[i],SCALE*pim[i]);
    if(Cmod(sparams2[i])==0.0)
      bad_data[i]=1;
      //printf("Empty actual data in index %i \n",i);
    //printf("%i %i %f %f \n",i,ndat,pr[i],pim[i]);
  }
  ...
  for(i=0;i<ntag;i++)
	...
	//printf("%i %i %i \n",i,j,k);
	dptr = (long)(i+ntag*(k+nrx*j));
	//if(Cabs(sparams2[dptr])>Cabs(sparams[dptr]))
	sparams2[dptr]=Csub(sparams2[dptr],sparams[dptr]); //DLH
	  //else
	  //sparams2[dptr]=Complex(0.0,0.0);
	
	fac = Cmul(rcal[k+nrx*j],tcal[i+ntag*j]);
	//fac=ccal[dptr];
	sparams2[dptr]=Cmul(sparams2[dptr],fac);	

	// normalize data using st. dev.
	if(stdev[dptr]>0.0)
	  sparams2[dptr]=RCdiv(sparams2[dptr],stdev[dptr]);
      }
	...
	for(j=0;j<nfreq;j++){
	  ...
	  for(i=0;i<ntag;i++){
	    for(k=0;k<nrx;k++){
	      ...
	      //amp=d1[l+NX*(m+NY*(n+NZ*i))]*d2[l+NX*(m+NY*(n+NZ*k))];
	      sincosf(pse,&sinx,&cosx);
	      pointer=dptr+(long)ndat*mptr;
	      //printf("%i %i %li \n",ndat,nmod,pointer);
	      pointer2=mptr+(long)nmod*dptr;
	      g[pointer]=Complex(cosx,sinx);
	      if(bad_data[dptr]==1 || bad_cal[dptr]==1){
		g[pointer]=Complex(0.0,0.0); // bad data DLH		
	      }	      
	      if(stdev[dptr]>0.0)
		g[pointer]=RCdiv(g[pointer],stdev[dptr]);
  ...
  for(l=0;l<NX;l++)
	...
	for(j=0;j<nfreq;j++) 
	  for(i=0;i<ntag;i++)
	    for(k=0;k<nrx;k++){
	      pointer=(long)(i+ntag*(k+nrx*j))
		+(long)ndat*(long)(l+NX*(m+NY*n));
	      pointer2=(long)(l+NX*(m+NY*n))
		+(long)nmod*(long)(i+ntag*(k+nrx*j));
  ...
  */  
  ...
  alpha = 1.0e4;  // 1.0e4, smaller ...5.0e2
  ...
  for(l=0;l<NX;l++)
	...
	if(image[n+NZ*(m+NY*l)]>thresh){
	  ic[n+NZ*(m+NY*l)]=1;
	  dimage[3*npoints]=(float)l;
	  dimage[3*npoints+1]=(float)m;
	  dimage[3*npoints+2]=(float)n;
	  npoints++;
	}
      }
  ...
  num=i4block_components(NZ, NY, NX, ic, id);
  ...
  for(l=0;l<NX;l++)
    for(m=0;m<NY;m++)
      for(n=0;n<NZ;n++)
	iclust[id[n+NZ*(m+NY*l)]]+=1;
  for(l=0;l<num;l++)
    if(iclust[l]>1) printf("%i %i \n",l,iclust[l]);
  ...
  for(l=0;l<NX;l++)
    for(m=0;m<NY;m++)
      for(n=0;n<NZ;n++)
	image[n+NZ*(m+NY*l)]=(float)id[n+NZ*(m+NY*l)];
  */
  ...
  srand(iseed);
  for(i=0;i<k;i++){
    frac=(float)i/(float)(k+1);
    ang=6.28*frac;
    centroid[3*i]=(float)(NX/2)*(1.0+0.5*cos(ang));
    centroid[3*i+1]=(float)(NY/2)*(1.0+0.5*sin(ang));
    centroid[3*i+2]=frac;
  }
  kmeans(l,dimage,npoints,k,centroid,final);
  ...
  for(l=0;l<NX+1;l++){
	...
	xx[n+(NZ+1)*(m+(NY+1)*l)]=x/4.0;
	yy[n+(NZ+1)*(m+(NY+1)*l)]=y/4.0;
	zz[n+(NZ+1)*(m+(NY+1)*l)]=z/4.0;
