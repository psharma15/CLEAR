%% This code generates objects at different locations in the imaging 
...
function [Alib,objCenterGrid] = genLibraryMPexpt(opts)
% -------------------------------------------------------------------------
% Input: 
...
% -----------------------------ARRANGEMENT---------------------------------
% Needs to be decided based on probable object locations, etc.
% -------------------------------------------------------------------------
...
if ~isfield(opts,'imSize')
    opts.imSize = [0, 3.6; 0.0, 3.6; 0.0, 2.4]; % meters
end
if ~isfield(opts,'voxSz')
    opts.voxSz = [0.05;0.05;0.3]; % meters
end
...
if ~isfield(opts,'A')
    [rxPosition,tagPosition,opts.freq] = posRxTxTrueSizeExpt(opts.posRxTx);
    opts.A = genA(opts.imSize,opts.voxSz,tagPosition,rxPosition,opts.freq);
end
if ~isfield(opts,'savePath')
    opts.savePath = ['E:\ArpaE2018\3DImaging_Simulation\CST_SimulationData',...
        'Analysis\Algorithms\ExptData\Guoyi_exptTrueSize_05292019\'];
end
...
% This function generates 5 x 5 grid points in entire room, with z = 0
%objSz = sort(objSz,'ascend');
zCoord = 0;
...
%objSz = sort(objSz,'ascend');
zCoord = 0;
xLim = [(imSz(1,1)), imSz(1,2)];
yLim = [(imSz(2,1)), imSz(2,2)];
numPtsXY = [5, 5];
xCoord = linspace(xLim(1),xLim(2),numPtsXY(1));
yCoord = linspace(yLim(1),yLim(2),numPtsXY(2));
